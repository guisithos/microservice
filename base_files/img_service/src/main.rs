...
/**
Microservice_handler fn handle 4 cases:
- return index page on /path
- store file to /upload
- return the file with /download
- return 404 erros to else
*/
...
/**
Get incoming files reading stream of request. 
Don't hold files in memory, read incoming data in chunks
and write to a file.  
*/
fn main() {
    //files dir to keep incoming files
    let files = Path::new("./files");
    fs::create_dir(files).ok();
    //Start server with the fn microservice_handler. 
