...
fn microservice_handler(req: Request<Body>, user_db: &UserDb)
    //Referecent to shared state
    -> impl Future<Item=Response<Body>, Error=Error>
{
    //Match expression detects returned method from req.method and URI from req.uri()
    let response = {
	...
	//we get an object stored in cap var.
		...
		//Add UserData instance to user DB, send user ID with ok status
		(&Method::POST, None) => {
		...
		//From user DB get ID, if found convert to str, else 404
                (&Method::GET, Some(id)) => {
                ...
                //get_mut search the user DB, deference * to replace data
                (&Method::PUT, Some(id)) => {
		...
		//Check if slabs contains data, then remove w/ remove method.
		//else 404. Need to check data b4 removing so it doesn't panic.
...
//FN creates empty responses, carrying out: expects status code, creates new 
//response builder, set status, adds empty body.
...
fn main() {
	...
	//Cloned reference to move to nested closure to be called n. times
